...
function consultar_mysql($tabela, $pesquiza=array(), $ordenar='', $orient='',$somas_in=''){
  ...
  if (is_array($tabela)){
    ...
    foreach ($tabela as $k => $v) {
       ...
       $tabela_vinc .= $direct . " JOIN ".$table2[0].' on '.$v[0]." = ".$v[1]   ;
       $tab[] = $table1[0];
  ...
  //Efetua soma
  if($somas_in){
    foreach ($somas_in as $key => $value) {
      $soma_cols[] = "$value[0]($value[1]) AS '$value[0]_$value[1]'";      
    }
    $soma_cols_rest = implode(',', $soma_cols);
    $somas = mysqli_query(conectarMysql(),"SELECT $soma_cols_rest FROM $tabela $tabela_vinc $pesquizar");
    $somas = mysqli_fetch_assoc($somas); 
  }
...
//Post Json
function post_json($url,$data=array(),$noarray=''){
...
//abrelink
function abrir_url($url, $post='',$cookie='cookie.txt',$headr=''){
...
function formatamoeda($number,$casas=2){
...
if($_POST[salvar]==tables_mysql or $_POST[salvar]==tables_mysqli_config){
  ...
  $saida = salvar_mysql($table,$_POST,$_POST[table_id],table_id);
...
if($_GET[exclui_mysql]){
	exclui_mysql($_GET[exclui_mysql],$_GET[id]);
	$saida['status']=ok;
	echo json_encode($saida);
	die;
}
...
if($_POST[edit]){
  ...
  function Post_uploads($pasta){
    if($_FILES){
      foreach ($_FILES as $key => $value) {
        if(!is_dir($pasta))mkdir($pasta);

        $exention = explode('.',$_FILES[$key]['name'])[1];
        $_FILES[$key][md5]  = md5($_FILES[$key]['name']).'.'.$exention;



        move_uploaded_file($_FILES[$key]['tmp_name'], $pasta.'/'.$_FILES[$key][md5]);
...
if($_POST[salvar]){
  ...
  ;
  //Salva Conteudo
  $salvar_mysql = salvar_mysql($tabela,$_POST,$_POST[$tabela."_id"],$tabela."_id");
...
if($_GET[json]){
  ...
  $filterRules = json_decode($_POST[filterRules],1);
  ...
  if(!$filterRules){
      ...
      foreach ($filterRules as $key => $value) {
        $buscar[$value[field]] = $value[value];
      }
      $dados = consultar_mysql($tabela,$buscar,$ordem,$direc);
  }
...
if($_GET[json_rows]){
  ...
  $filtro = $_GET[json_rows];
  unset($_GET[filtro]);

  $tabela = $_GET[json_rows];
  unset($_GET[json_rows]);
  $direc = $_GET[direc];
  unset($_GET[direc]);
  ...
  $dados = consultar_mysql($tabela,$_GET,$ordem,$direc)[rows];
...
function downloads($file,$file_save){
    if(!file_exists($file_save)){
        $save = file_get_contents($file);
        file_put_contents($file_save, $save);
        return $file_save;
    }
    return false;
}
...
function date_to_mktime($date){
    ...
    $data_hora = explode(' ', $date);
    $data = explode('-', $data_hora[0]);
    $hora = explode(':', $data_hora[1]);

    return mktime($hora[0],$hora[1],$hora[3],$data[1],$data[2],$data[0]);
}

function date_to_mktime_br($date){

    $data_hora = explode(' ', $date);
    $data = explode('/', $data_hora[0]);
    $hora = explode(':', $data_hora[1]);

    return mktime($hora[0],$hora[1],$hora[3],$data[1],$data[0],$data[2]);
}
...
function codigoUnico(){
    return md5(mktime().rand(0,99999999));
}
...
function list_files($path,$extention=''){
    $scandir = scandir($path);
    foreach ($scandir as $key => $value) {  
        if(($value != '..' && $value != '.') ){
            $file = pathinfo($path.'/'.$value);
...
function formularios($fields){
    ...
    foreach ($fields as $key => $value) {
        ...
        if(!$value[labelWidth])$value[labelWidth] = 140;
        if(!$value[style])$value[style] = 'width:400px;';
        if(
          $value[padrao] and 
          !$value[value]
        ){
          $value[value] = $value[padrao];
        }


        $class = 'class="'.$value['class'].'" ';unset($value['class']);
        ...
        $fieldsCompile .= '
        <div class="fields"><input '.$class.$tags.'></div>';

   }

    return $fieldsCompile;
