		...
		$(target).find('>div').each(function(){	// each column panel
			...
			column.find('>div').each(function(){	// each portal panel
				...
				popts.onMaximize = function(){
					...
					popts.stub = $('<div></div>').insertAfter($(this).panel('panel'));
					$(this).panel('panel').addClass('portal-maximized').appendTo(target);
					$(this).panel('resize', {top:top,left:0,width:width,height:height})
					onMaximize.call(this);
				};
				popts.onRestore = function(){
					$(target).removeClass('portal-noscroll');
					$(this).panel('panel').removeClass('portal-maximized').insertAfter(popts.stub);
					popts.stub.remove();
					popts.stub = null;
					onRestore.call(this);
				};
