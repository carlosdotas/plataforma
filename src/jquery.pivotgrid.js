...
(function($){
	function create(target){
		...
		opts.pivot.saves = JSON.parse(localStorage.getItem('relatorios')) || {};
		// var filterRules = {};
	...
	function aggregateColumn(target, rows){
		...
		if (opts.pivot.aggregate && opts.pivot.aggregate.column){
			...
			$.map(rows, function(row){
				...
				row[opts.pivot.aggregate.column.field] = value.toFixed(opts.valuePrecision);
			});
		}
		return rows;
	}
	...
	function aggregateFooter(target, rows){
		...
		if (opts.pivot.aggregate && opts.pivot.aggregate.footer){
			...
			for(var i=0; i<fields.length; i++){
				...
				footerRow[fields[i]] = value.toFixed(opts.valuePrecision);
			}
			return [footerRow];
		}
		return [];
			...
			state.layoutDialog.dialog({
				...
				buttons:[{
					...
					handler:function(){
						var save = {}
						var relatorios = []
						var name = prompt("Digite um nome para o relatório");

						if(name){
							save.name = name;
							save.filters = getFields('filters');
							save.rows = getFields('rows');
							save.columns = getFields('columns');
							save.values = getFields('values');

							if(localStorage.getItem('relatorios')){
								relatorios = JSON.parse(localStorage.getItem('relatorios'));
							}

							relatorios.push(save);
							opts.pivot.saves = relatorios;
							localStorage.setItem('relatorios', JSON.stringify(relatorios));

							if(opts.onSaveRelatorios){
								opts.onSaveRelatorios(relatorios)
							}
						}					
					}
				},{
	...
	$.fn.pivotgrid.methods = {
		...
		saves: function(jq){
			return $.data(jq[0], 'pivotgrid').options.pivot.saves;
		},		
		getData: function(jq){
